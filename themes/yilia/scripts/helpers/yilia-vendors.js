...
'use strict';
// 自定义控制台打印函数
const yilia_log = require('../yilia_log');

/** 自定义静态资源链接生成 */
hexo.extend.helper.register('yilia_vendors', function (url) {
  if (!!url && url.startsWith('//')) return url;
  return this.url_for(`${url}`);
});

/** 自定义网站链接生成 */
hexo.extend.helper.register('yilia_full_url', function (url) {
  ...
  yilia_plus_log("yilia_full_url finish ===> " + fullUrl);
  return fullUrl;
});
